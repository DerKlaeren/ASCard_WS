...
router.put("/:id", async (req, res) => {
	...
	if (game) {
        ...
        db.pool.query("UPDATE asc_game SET gameid=?, ownerPlayerId=?, title=?, background=?, era=?, yearInGame=?, accessCode=?, locked=?, scheduled=?, started=?, finished=?, createdAt=? WHERE gameid = ?", [updated.Object.values(updated), game.gameid], (error, result) => {
